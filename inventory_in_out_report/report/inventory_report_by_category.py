...
class inventory_report_category(models.AbstractModel):
    _name = 'report.inventory_in_out_report.inventory_report_by_category'
    ...
    # def _get_scrap_quantity(self, data, categ_id):
    #     location_id = data['form'] and data['form'].get('location_id') or False
    #     start_date = self.convert_withtimezone(data['form']['start_date']+' 00:00:00')
    #     end_date =  self.convert_withtimezone(data['form']['end_date']+' 23:59:59')
    #     product_tmpl_ids = self.env['product.template'].search([('categ_id','=',categ_id)])
    #     for pro in product_tmpl_ids:
    #         product_id = self.env['product.product'].search([('product_tmpl_id','=',pro.id)]).id
    #         # scrap_id = self.env['stock.scrap'].search([('product_id','=',product_id.id),('location_id','=',location_id),('create_date','>=',start_date),('create_date','<=',end_date)])
    #         self._cr.execute('''
    #                         SELECT  
    #                             sum(ss.scrap_qty) AS quant 
    #                         FROM stock_scrap as ss
    #                         LEFT JOIN product_product as pp ON(pp.id = ss.product_id)
    #                         LEFT JOIN product_template as pt ON(pt.id = pp.product_tmpl_id)
    #                         LEFT JOIN product_category as pcat ON(pcat.id = pt.categ_id)
    #                         WHERE ss.product_id = %s 
    #                         AND ss.location_id = %s
    #                         AND ss.create_date >= %s
    #                         AND ss.create_date <= %s
