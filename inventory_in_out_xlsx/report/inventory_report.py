...
class inventory_report(models.AbstractModel):
    _name = 'report.inventory_in_out_xlsx.inventory_report_by_warehouse'
    ...
    def _total_adj(self):
        ...
        """
        print ("Adjustment",self.total_adj)
    ...
    def location_wise_value(self, start_date, end_date, locations , include_zero=False,filter_product_ids=[]):
        ...
        self._cr.execute('''
                        SELECT pp.id AS product_id,
                            ...
                            sum((
                                CASE WHEN spt.code in ('incoming') AND sm.location_dest_id in %s AND sourcel.usage !='inventory' AND destl.usage !='inventory' AND sm.location_id = swo.location_id
                                THEN (sm.product_qty * pu.factor / pu2.factor)
                                ELSE 0.0 
                                END
                            )) AS product_qty_in,
                            sum((
                                CASE WHEN (spt.code ='internal' or spt.code is null) AND sm.location_dest_id in %s AND sourcel.usage !='inventory' AND destl.usage !='inventory' AND sm.location_id = swo.location_id
                                THEN (sm.product_qty * pu.factor / pu2.factor)  
                                WHEN (spt.code='internal' or spt.code is null) AND sm.location_id in %s AND sourcel.usage !='inventory' and destl.usage !='inventory' AND sm.location_id = swo.location_id
                                THEN -(sm.product_qty * pu.factor / pu2.factor)
                                ELSE 0.0 
                                END
                            ...
                            sum((
                                CASE WHEN sourcel.usage = 'inventory' AND sm.location_dest_id in %s  AND swo.location_id = sm.location_dest_id
                                THEN  (sm.product_qty * pu.factor / pu2.factor)
                                WHEN destl.usage ='inventory' AND sm.location_id in %s AND swo.location_id = sm.location_dest_id
                                THEN -(sm.product_qty * pu.factor / pu2.factor)
                                ELSE 0.0 
                                END
                            )) AS product_qty_adjustment,
                            sum((
                                CASE WHEN swo.location_id in %s AND swo.location_id = sm.location_dest_id
                                THEN (swo.product_min_qty * pu.factor / pu2.factor)
                                ELSE 0.0 
                                END
                            )) AS product_qty_min,
                            sum((
                                CASE WHEN swo.location_id in %s AND swo.location_id = sm.location_dest_id
                                THEN (swo.product_max_qty * pu.factor / pu2.factor)
                                ELSE 0.0 
                                END
                            )) AS product_qty_max,
                            sum((0.0)) AS under_stock,
                            sum((0.0)) AS over_stock
    ...
    def get_xlsx(self, options,response):        
        ...
        sheet.merge_range(y_offset, 3, y_offset, 4, _('Inventory Report'), self.format_title)
        y_offset += 2
        
        sheet.merge_range(y_offset, 1, y_offset, 2, _('Company'), self.format_header_center)
        sheet.set_column(1, 2, 20)
        sheet.merge_range(y_offset, 3, y_offset, 4, _('Warehouse'), self.format_header_center)
        sheet.set_column(3,4, 20)
        sheet.merge_range(y_offset, 5, y_offset, 6, _('Date'), self.format_header_center)
        sheet.set_column(5, 6, 20)
        sheet.write(y_offset, 7, _('Sort By'), self.format_header_center)
        sheet.set_column(7, 7, 20)
        y_offset += 1

        sheet.merge_range(y_offset, 1, y_offset, 2, company_name or '', self.format_title_data)
        sheet.set_column(1, 2, 20)
        sheet.merge_range(y_offset, 3, y_offset, 4, warehouse_name or '', self.format_title_data)
        sheet.set_column(3, 4, 20)
        sheet.merge_range(y_offset, 5, y_offset, 6, df or 'All', self.format_title_data)
        sheet.set_column(5, 6, 20)
        sheet.write(y_offset, 7, options['form']['sort_order'] or '', self.format_title_data)
        sheet.set_column(7, 7, 20)            
        y_offset += 2
        
        lines=self._get_lines(options,options['form']['company_id'])
        
        sheet.merge_range(y_offset, 0, y_offset, 2, _('Product'), self.format_header_one)
        sheet.set_column(0, 2, 20)
        sheet.write(y_offset, 3,_('Beginning'), self.format_header_one)     
        sheet.set_column(3, 3, 20)
        sheet.write(y_offset, 4, _('In'), self.format_header_one)
        sheet.set_column(4,4, 20)
        sheet.write(y_offset, 5, _('Out'), self.format_header_one) 
        sheet.set_column(5,5, 20)   
        sheet.write(y_offset, 6, _('Internal'), self.format_header_one) 
        sheet.set_column(6, 6, 20)   
        sheet.write(y_offset, 7, _('Adjustment'), self.format_header_one) 
        sheet.set_column(7, 7, 20)          
        sheet.write(y_offset, 8, _('Ending'), self.format_header_one) 
        sheet.set_column(8, 8, 20)  
        sheet.write(y_offset, 9, _('Min Qty'), self.format_header_one) 
        sheet.set_column(9, 9, 20) 
        sheet.write(y_offset, 10, _('Max Qty'), self.format_header_one) 
        sheet.set_column(10, 10, 20)  
        sheet.write(y_offset, 11, _('Over Stock'), self.format_header_one) 
        sheet.set_column(11, 11, 20)  
        sheet.write(y_offset, 12, _('Under Stock'), self.format_header_one) 
        sheet.set_column(12, 12, 20)  
        y_offset += 1     
        for line_wh_id in lines:
            for line_id in lines[line_wh_id]:
                begin_qty=self._get_beginning_inventory(options,line_wh_id,line_id.get('product_id'),line_id)
                onhand_qty = (begin_qty + line_id.get('product_qty_in', 0.0) + line_id.get('product_qty_out', 0.0) + line_id.get('product_qty_internal', 0.0) + line_id.get('product_qty_adjustment',0.0)) or 0.0                    
                if line_id.get('product_qty_max') == None:
                    line_id['product_qty_max']=0.0
                if line_id.get('product_qty_min') == None:
                    line_id['product_qty_min']=0.0
                if onhand_qty > line_id.get('product_qty_max', 0.0):
                    line_id.update({'over_stock': onhand_qty - line_id.get('product_qty_max', 0.0)}) 
                if onhand_qty < line_id.get('product_qty_max', 0.0):
                    line_id.update({'under_stock': line_id.get('product_qty_max', 0.0) - onhand_qty})
        total_minqty = 0.0
        total_maxqty = 0.0
        total_ovstock_total = 0.0
        total_under_stock = 0.0   
        for line_wh in lines:
            min_qty_total = 0.0
            max_qty_total = 0.0
            over_sock_total = 0.0
            under_stock_total = 0.0
            for line in lines[line_wh]:
                ...
                sheet.merge_range(y_offset, 0, y_offset, 2, product_name or '', self.product_format)
                sheet.set_column(0, 2, 20)                
                sheet.write(y_offset,3, begin_qty or 0.0, self.format_border_top)
                sheet.set_column(3, 3, 20)            
                sheet.write(y_offset, 4, line.get('product_qty_in', 0.0) or 0.0, self.format_border_top)
                sheet.set_column(4, 4, 20)
                sheet.write(y_offset, 5, line.get('product_qty_out', 0.0) or 0.0, self.format_border_top)
                sheet.set_column(5,5, 20)
                sheet.write(y_offset, 6, line.get('product_qty_internal', 0.0) or 0.0, self.format_border_top)
                sheet.set_column(6, 6, 20)
                sheet.write(y_offset, 7, line.get('product_qty_adjustment', 0.0) or 0.0, self.format_border_top)
                sheet.set_column(7,7, 20)
                sheet.write(y_offset, 8, (begin_qty + line.get('product_qty_in', 0.0) + line.get('product_qty_out', 0.0) + line.get('product_qty_internal', 0.0) + line.get('product_qty_adjustment', 0.0)) or 0.0, self.format_border_top)
                sheet.set_column(8, 8, 20)
                sheet.write(y_offset, 9, line.get('product_qty_min', 0.0) , self.format_border_top)
                sheet.set_column(9,9, 20)
                sheet.write(y_offset, 10, line.get('product_qty_max', 0.0) , self.format_border_top)
                sheet.set_column(10, 10, 20)
                sheet.write(y_offset, 11, line.get('over_stock', 0.0) , self.format_border_top)
                sheet.set_column(11, 11, 20)
                sheet.write(y_offset, 12, line.get('under_stock', 0.0) , self.format_border_top)
                sheet.set_column(12, 12, 20)
                y_offset += 1
                min_qty_total += line.get('product_qty_min', 0.0)
                max_qty_total += line.get('product_qty_max', 0.0)
                over_sock_total += line.get('over_stock', 0.0)
                under_stock_total += line.get('under_stock', 0.0)          
            if line_wh:            
            ...
            sheet.merge_range(y_offset, 0, y_offset, 2, warehouse_name_group or '', self.format_header_one)    
            sheet.set_column(0, 2, 20) 
            sheet.write(y_offset,3,self._total_begin() or 0.0, self.format_border_top)
            sheet.set_column(3, 3, 20)            
            sheet.write(y_offset, 4,self._total_in() or 0.0, self.format_border_top)
            sheet.set_column(4, 4, 20)
            sheet.write(y_offset, 5, self._total_out() or 0.0, self.format_border_top)
            sheet.set_column(5,5, 20)
            sheet.write(y_offset, 6, self._total_int() or 0.0, self.format_border_top)
            sheet.set_column(6, 6, 20)
            sheet.write(y_offset, 7, self._total_adj() or 0.0, self.format_border_top)
            sheet.set_column(7,7, 20)
            sheet.write(y_offset, 8, self._total_end() or 0.0, self.format_border_top)
            sheet.set_column(8, 8, 20)
            sheet.write(y_offset, 9, min_qty_total , self.format_border_top)
            sheet.set_column(9, 9, 20)
            sheet.write(y_offset, 10, max_qty_total, self.format_border_top)
            sheet.set_column(10, 10, 20)
            sheet.write(y_offset, 11, over_sock_total, self.format_border_top)
            sheet.set_column(11, 11, 20)
            sheet.write(y_offset, 12, under_stock_total, self.format_border_top)
            sheet.set_column(12, 12, 20)
            y_offset += 2
            total_minqty += min_qty_total
            total_maxqty += max_qty_total
            total_ovstock_total +=over_sock_total
            total_under_stock += under_stock_total
        total_toal_val=self._total_vals(cid[0])
        sheet.merge_range(y_offset, 0, y_offset, 2, 'Total Inventory', self.format_header_one)   
        sheet.set_column(0, 2, 20)
        sheet.write(y_offset,3, self._total_vals(cid[0])[0] or 0.0, self.format_border_top)
        sheet.set_column(3, 3, 20)            
        sheet.write(y_offset, 4,self._total_vals(cid[0])[1] or 0.0, self.format_border_top)
        sheet.set_column(4, 4, 20)
        sheet.write(y_offset, 5, self._total_vals(cid[0])[2] or 0.0, self.format_border_top)
        sheet.set_column(5,5, 20)
        sheet.write(y_offset, 6, self._total_vals(cid[0])[3] or 0.0, self.format_border_top)
        sheet.set_column(6, 6, 20)
        sheet.write(y_offset, 7, self._total_vals(cid[0])[4] or 0.0, self.format_border_top)
        sheet.set_column(7,7, 20)
        sheet.write(y_offset, 8, self._total_vals(cid[0])[5] or 0.0, self.format_border_top)
        sheet.set_column(8, 8, 20)
        sheet.write(y_offset, 9, total_minqty, self.format_border_top)
        sheet.set_column(9, 9, 20)
        sheet.write(y_offset, 10,  total_maxqty, self.format_border_top)
        sheet.set_column(10, 10, 20)
        sheet.write(y_offset, 11,  total_ovstock_total, self.format_border_top)
        sheet.set_column(11, 11, 20)
        sheet.write(y_offset, 12,  total_under_stock, self.format_border_top)
        sheet.set_column(12, 12, 20)
        y_offset += 1   
        workbook.close()
        output.seek(0)
    ...
    def xlsx_export(self,datas):  
        ...
        return {
                ...
                'data': {'model': 'report.inventory_in_out_xlsx.inventory_report_by_warehouse',
