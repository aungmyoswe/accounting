...
class inventory_report_category(models.AbstractModel):
    _name = 'report.inventory_in_out_xlsx.inventory_report_by_category'
    ...
    def get_xlsx(self, options,response):        
        ...
        for line_wh_id in lines:
            for line_id in lines[line_wh_id]:
                begin_qty=self._get_beginning_inventory(options,cid[0],line_id.get('product_id'),line_id)
                onhand_qty = (begin_qty + line_id.get('product_qty_in', 0.0) + line_id.get('product_qty_out', 0.0) + line_id.get('product_qty_internal', 0.0) + line_id.get('product_qty_adjustment',0.0))
                ...
                if onhand_qty > line_id.get('product_qty_max'):
                    line_id.update({'over_stock': onhand_qty - line_id.get('product_qty_max')}) 
                if onhand_qty < line_id.get('product_qty_min'):
                    line_id.update({'under_stock': line_id.get('product_qty_min') - onhand_qty})
        ...
        for line_pc in lines:
            ...
            sheet.merge_range(y_offset, 0, y_offset, 2, pcategory_name_group or '', self.format_header_one)    
            sheet.set_column(0, 2, 20) 
            ...
            sheet.set_column(8, 8, 20)
            sheet.write(y_offset, 9, 0.0, self.format_border_top)
            sheet.set_column(9, 9, 20)
            sheet.write(y_offset, 10, 0.0, self.format_border_top)
            sheet.set_column(10, 10, 20)
            sheet.write(y_offset, 11, 0.0, self.format_border_top)
            sheet.set_column(11, 11, 20)
            sheet.write(y_offset, 12, 0.0, self.format_border_top)
            sheet.set_column(12, 12, 20)
            y_offset += 2
    ...
    def xlsx_export(self,datas):  
        ...
        return {
                ...
                'data': {'model': 'report.inventory_in_out_xlsx.inventory_report_by_category',
